...
export function renderPreview(container, project, variables) {
  ...
  const header = createElement('div', '', {
    html: `
      <h2 style="margin:0 0 4px; font-size:22px;">${project.name || 'Formulario Clínico'}</h2>
      <div style="color:#64748b; font-size:13px; margin-bottom:24px;">
        ${project.specialty || ''} • ${project.date || ''}
      </div>
    `
  });
  ...
  variables.forEach((variable, index) => {
    ...
    block.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
          <strong style="font-size:15px;">${variable.name}</strong>
          <div style="font-size:12px; color:#64748b; margin-top:2px;">
            ${variable.metadata?.question || variable.description || ''}
          </div>
        </div>
